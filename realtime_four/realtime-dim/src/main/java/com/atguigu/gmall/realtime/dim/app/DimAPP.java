...
//        }).map(JSONObject::parseObject);
        ...
        MySqlSource<String> mySqlSource = FlinkSourceUtil
                .getMySqlSource(Constant.PROCESS_DATABASE, Constant.PROCESS_DIM_TABLE_NAME);
...
//        // 7. 写出到HBase
    ...
    public SingleOutputStreamOperator<Tuple2<JSONObject, TableProcessDim>> connectionStream
            (SingleOutputStreamOperator<JSONObject> jsonObjStream, MapStateDescriptor<String,
                    TableProcessDim> broadcastState, BroadcastStream<TableProcessDim> broadcastStateStream) {
        BroadcastConnectedStream<JSONObject, TableProcessDim> connectStream = jsonObjStream.connect(broadcastStateStream);

        return connectStream
                .process(new DimBroadcastFunction(broadcastState)).setParallelism(1);
    }
    ...
    public SingleOutputStreamOperator<JSONObject> etl(DataStreamSource<String> stream) {
        ...
        }).returns(JSONObject.class);  // 使用lambda表达式无法推断的时候最后可以加.returns设置数据类型
    }
}
